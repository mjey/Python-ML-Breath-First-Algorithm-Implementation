...
class State(object):
	...
	def SUCCESSORS(self):
		listChild = []
		if not self.IS_Valid() or self.IS_Goal_State():
			return listChild
		...
		for i in self.moves:
			(m, c) = i
			self.Adds_Valid_Successors(listChild, m, c, sgn, direction)
		return listChild

	def Adds_Valid_Successors(self, listChild, m, c, sgn, direction):
